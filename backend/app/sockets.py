...
class ConnectionManager:
    ...
    async def send_event(self, connection: WebSocket, event_type: str, data: dict[str, any]):
        await connection.send_json({
            "type": event_type,
            "data": data
        })

    async def broadcast_event(self, lobby_id: str, event_type: str, data: dict[str, any]):
        for connection in self.lobby_to_connections[lobby_id]:
            await self.send_event(connection, event_type, data)

    async def handle_player_join(self, connection: WebSocket, lobby_id: str, player_id: str, player_name: str):
        ...
        await self.broadcast_event(lobby_id, "PLAYER_JOIN", {
            "playerId": player_id,
        ...
        await self.send_event(connection, "LOBBY_STATE", {
    ...
    async def handle_player_leave(self, connection: WebSocket):
        ...
        database = connection.app.database["Lobby"]
        database.update_one({"_id": metadata.lobby_id}, {"$pull": {"players": {"id": metadata.player_id}}})

        await self.broadcast_event(metadata.lobby_id, "PLAYER_LEAVE", {
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            match event_type:
                case "PLAYER_JOIN":
                    await manager.handle_player_join(websocket, event_data["lobbyId"], event_data["playerId"], event_data["playerName"])
                case _:
